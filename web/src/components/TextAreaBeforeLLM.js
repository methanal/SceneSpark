...
const TextAreaAfterLLM = ({ uniqueID, handleFetchTab4 }) => {
  ...
  const fetchPrompts = async (uniqueID) => {
    try {
      ...
      if (response.ok) {
        ...
        setText3(result.vision_with_srt_prompt || '');
  ...
  return (
    <>
      ...
      <TextArea
        ...
        style={{ width: '100%' }}
      />
      <Button onClick={() => handleFetchTab4(translationModel, modelSize, whisper_prompt, samplingInterval, clipDuration, text3)} style={{ marginTop: '16px' }}>Extract Clips</Button>
    </>
  );
};

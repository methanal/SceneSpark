...
PROMPT_PICK_VIDEO_META_ALL_RETURN_JSON = """我将提供很多截图和字幕，这些截图和字幕来自同一短剧视频文件。

截图消息与它前面的字幕消息是对应的，有的截图消息没有对应的字幕消息。
...
你需要根据这些截图和对应的字幕，做如下事情：
1. 通过字幕和截图，还有截图里的动作，人物表情等内容，理解和分析剧情
2. 按照剧情重要度，关键剧情转折点，关键动作/人物特征表情，冲突程度等重要信息，选中重要的、关键的截图。
3. 针对选中的截图，给出：截图索引、标签、图片描述，标记 pick: True
4. 针对没有被选中的截图，给出：截图索引，标记 pick: False

回复给我时，注意一下几点：
1. 返回格式，仅返回给我 JSON，不要返回其它内容。仅返回可以直接由 Python 解析的 JSON 内容，不要用```json```包住。
2. 针对选中的截图，返回数据内，包括：截图索引、标签、图片描述、pick=True
3. 针对没有选中的截图，返回数据内，包括：截图索引、pick=False
4. 注意，截图索引从 0 开始，不要从 1 开始。比如，第 1 张图，索引就是 0。
5. 被你选中的截图条数，不要超过原总截图数的{selection_ratio}；

举例：
我发给你 4 张截图和截图对应的字幕，
你首先根据截图和截图对应的字幕，理解和分析剧情，按照剧情关键度，剧情转折度，关键动作/表情，冲突程度来筛选出关键截图。
譬如说选出了第 2、4 张截图，并针对每张截图给出分类标签tag，内容关键度评级（rating）打分（1-5分），截图关键动作/表情描述（movement），内容冲突（confliction）打分（1-5分）、剧情转折（changing）打分（1-5分），生成图片描述，并把画面字幕信息放入subtitle里

最后仅返回给我 JSON 格式的数据，格式和示例如下：
{{
  "data": [
    {{"index": 0, "pick": false}},
    {{"index": 1, "pick": true, "tags": ["情侣","不治之症","医院"], "rating":4, "movement": ["阴笑","打耳光"],"confliction": 3,"changing": 4, "description": "一名男子发现女子吐血，女子可能有遗传疾病","subtitle": "我家里有遗传病，没有办法治"}},
    {{"index": 2, "pick": false}},
    {{"index": 3, "pick": true, "tags": ["下毒","奸笑","医生"], "rating":5, "movement": ["得意大笑","鼓掌"],"confliction": 2,"changing": 2,  "description": "一名医生在奸笑，说毒是他下的。","subtitle": "看来软骨散奇效了，你这次完蛋了！"}}
  ]
}}
"""

PROMPT_PICK_VIDEO_META_ALL_STEP1_RETURN_JSON = """我将提供很多截图和字幕，这些截图和字幕来自同一短剧视频文件。
...
你需要根据这些截图和对应的字幕，做如下事情：
1. 通过字幕和截图，还有截图里的动作，人物表情等内容，理解和分析剧情
2. 按照剧情重要度，关键剧情转折点，关键动作/人物特征表情，冲突程度等重要信息，对每张图做标注，标注为“重要”或“次要”。
3. 你认重要的截图，标记 important: true
4. 你认次要的截图，标记 important: false

回复给我时，注意一下几点：
1. 返回格式，仅返回给我 JSON，不要返回其它内容。仅返回可以直接由 Python 解析的 JSON 内容，不要用```json```包住。
2. 每张图都需要标注，不能多，也不能缺失图片。
3. 注意，截图索引从 0 开始，不要从 1 开始。比如，第 1 张图，索引就是 0。
4. 你认为重要的截图条数，不要超过原总截图数的{selection_ratio}；

举例：
我发给你 4 张截图和截图对应的字幕，
你首先根据截图和截图对应的字幕，理解和分析剧情，按照剧情关键度，剧情转折度，关键动作/表情，冲突程度来标记截图。
比如，你认为第 2、4 张截图重要，第 1、3 张截图次要，则返回：
最后仅返回给我 JSON 格式的数据，格式和示例如下：
{{
  "data": [
    {{"index": 0, "important": false}},
    {{"index": 1, "important": true}},
    {{"index": 2, "important": false}},

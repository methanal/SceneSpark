...
from clippers.wrappers.openai_wrapper import initialize_openai_client, llm_pick_imgs
...
class LLMVisionClipper(BaseClipper):
    ...
    def extract_clips(self, video_path: Path, prompt: str) -> List[Dict]:
        ...
        encode_frames = LLMVisionClipper.sample_frames(video_path, interval=20.0)
        end_time_sample = time.monotonic()
        LOGGER.debug(
            f"Sample Elapsed: {end_time_sample - start_time_sample}s"  # noqa: G004
        )

        _imgs_json = llm_pick_imgs(  # noqa: F841  # TODO
            self.llm_client, encode_frames, prompt
        )
